...
""" Queue system  """
class server_queue:
	def __init__(self, env, arrival_rate, Packet_Delay, Server_Idle_Periods, sizeofbuffer):
	...
	def packets_arrival(self, env):
		...
		while True:
			...
			if self.queue_len < self.sizeofbuffer:
...
def main():
	...
	for arrival_rate in [0.2, 0.4, 0.6, 0.8, 0.9, 0.99]:
		...
		print("simulated loss probability is", round(router.lostpkts/float(router.packet_number),8))
		theoretical = (1-p)/(1-(p)**(router.sizeofbuffer + 1))*((p)**router.sizeofbuffer)
		print("theoretical loss probability is", round(theoretical,8))
